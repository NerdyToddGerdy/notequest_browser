...
/**
 * Climate transitions (issue #107). How many hexes of latitude separate the temperate middle from
 * the first cold hex -- so a character starting in a hot band reaches tundra about 8 hexes north or
 * south of home. Chosen as a pacing number, not a rules one: provisions cap at 20 and cost 1-3 per
 * hex, so 8 is roughly one full load in good terrain and a resupply in bad.
 *
 * **This whole latitude system is not a NoteQuest rule.** The rulebook prints Hot and Cold as two
 * alternative tables for two kinds of continent (rules 854-874) and never describes a boundary
 * inside one map. A world containing both is this project's own extension, the same tier as
 * `placeChild`'s collision-avoiding dungeon layout -- and it exists because #101's world-scoped
 * choice left 3 of the 10 built dungeon types, 4 Animals, 2 locations and 4 Event rows permanently
 * unreachable in any given save.
 */
export const COLD_LATITUDE = 8;

/** A deterministic, low-frequency wobble on the boundary, in hexes, so the climate line reads as
 * geography rather than as the straight row of hexes the raw latitude test would draw. A pure
 * function of `q` -- no seed, no persisted state -- which is what keeps `climateAt()` reproducible
 * for any coordinate without the world having to remember anything. */
function boundaryWobble(q: number): number {
  return Math.round(1.5 * Math.sin(q * 0.6) + 0.8 * Math.sin(q * 0.21));
}

/**
 * Which climate band a coordinate falls in -- the heart of option A ("latitude bands") from #107.
 *
 * Climate is a pure function of position, so there is **no new persisted state**: nothing to
 * migrate, no per-tile field, and two neighbours can never disagree about their own climate
 * depending on which was revealed first (the failure mode of the per-tile-drift alternative). It
 * also means an existing hot save simply *gains* cold territory it never walked to, rather than
 * needing a hard reset -- the headline win over #101's world-scoped shape.
 *
 * `WorldState.climate` keeps a real meaning: it says which band *home* sits in, which is what
 * #101's Character Creation picker now chooses. A hot world puts the temperate zone on home's own
 * latitude with cold in both directions; a cold world puts home on a pole, with the temperate zone
 * `COLD_LATITUDE` hexes away and a second pole beyond it.
 */
export function climateAt(coord: HexCoord, home: HexCoord, homeClimate: Climate): Climate {
  // Where the temperate middle sits. A hot world puts it on home's own row. A cold world puts home
  // on a pole instead -- and at 2x COLD_LATITUDE, not 1x, so that home sits as deep inside its own
  // band as a hot-world home does (8 hexes from the nearest boundary either way) rather than
  // balanced right on the edge of it.
  const equatorR = homeClimate === "hot" ? home.r : home.r + 2 * COLD_LATITUDE;
  const distance = Math.abs(coord.r - equatorR);
  return distance >= COLD_LATITUDE + boundaryWobble(coord.q) ? "cold" : "hot";
}

/** `climateAt()` bound to a world -- the form every reveal site wants, since they all resolve
 * climate per *neighbour* now rather than once per world. */
export function worldClimateAt(world: WorldState, coord: HexCoord): Climate {
  return climateAt(coord, world.home, world.climate);
}

/**
 * `climate` is the band the *neighbour being revealed* falls in, which as of #107 need not be the
 * band its parent sits in -- that's precisely what a transition is.
 *
 * The lookup used to be safe only by construction (every tile in a world came from this function
 * against one fixed climate). With both bands in one map that invariant is gone: a `swamp` parent
 * read against `COLD_TERRAIN_TABLE` would return `undefined`, since the `!` guards the roll index
 * and not the terrain key, and that `undefined` would flow straight into a `HexTile.terrain` and on
 * into `DUNGEON_TYPE_BY_TERRAIN`/`EVENT_TABLE` lookups. So the lookup is total now: a parent whose
 * terrain has no column in the target band's table is resolved against **its own** band instead.
 *
 * In practice that fallback is rare -- crossing a boundary almost always steps off one of the four
 * shared terrains -- and it degrades in the right direction: one extra hex of the old climate
 * before the new band takes hold, rather than an invalid tile.
 */
function nextTerrain(
  ...
  // Resolve the band whose table actually has a column for `current`, preferring the one asked for.
  // A realm terrain (issue #105) belongs to neither and never reaches here -- realms generate
  // through `revealRealmNeighborsInPlace()`, which has its own flat tables -- but falling back to a
  // shared terrain keeps this total for every possible input rather than only every reachable one.
  const band: Climate = terrainBelongsToClimate(current, climate)
    ? climate
    : terrainBelongsToClimate(current, "hot")
      ? "hot"
      : terrainBelongsToClimate(current, "cold")
        ? "cold"
        : climate;
  const parent: Terrain = terrainBelongsToClimate(current, band) ? current : "plain";
  const rolled =
    band === "hot"
      ? HOT_TERRAIN_TABLE[roll]![parent as HotTerrain]
      : COLD_TERRAIN_TABLE[roll]![parent as ColdTerrain];
...
export function revealNeighborsInPlace(
  tiles: Record<string, HexTile>,
  of: HexCoord,
  /** Climate transitions (issue #107): resolved per *neighbour*, not once per world, since a single
   * map now spans both bands and a reveal can straddle the boundary. Callers pass
   * `worldClimateAt`-bound closures; tests can pass `() => "hot"` for the old fixed behaviour. */
  climateOf: (coord: HexCoord) => Climate,
  rng: RNG,
  plainsAsWater = false,
): void {
  ...
  for (const neighbor of hexNeighbors(of)) {
    ...
    const terrain = nextTerrain(parent.terrain, climateOf(neighbor), rollDie(rng), plainsAsWater);
...
export function withPlayerMovedTo(world: WorldState, dest: HexCoord, rng: RNG): WorldState {
  ...
  revealNeighborsInPlace(
    tiles,
    dest,
    (coord) => worldClimateAt(world, coord),
    rng,
    world.plainsRevealAsWater,
  );
...
export function findOrRevealCompatibleHome(
  ...
  for (let ring = 0; ring < MAX_HOME_SEARCH_RINGS; ring++) {
    for (const coord of Object.keys(tiles).map(parseHexKey)) {
      // Issue #107: this is the one existing path that can genuinely cross a climate boundary --
      // it expands up to MAX_HOME_SEARCH_RINGS (20) rings, well past COLD_LATITUDE (8). That's
      // fine and even desirable: an Orc/Ogre whose nearest compatible city lies in the cold simply
      // starts there, and per-neighbour climate keeps the terrain coherent on the way out.
      revealNeighborsInPlace(
        tiles,
        coord,
        (c) => worldClimateAt(world, c),
        rng,
        world.plainsRevealAsWater,
      );
    }
 ...
 * fully authored. It's now chosen once, when the world itself is first created -- and `WorldState`
 * outlives every character, so this is a world-scoped decision rather than a per-character one.
 * `CharacterCreationScreen` only offers the choice when there's no world yet (a first adventurer,
 * or the first after a hard reset).
 *
 * Issue #107 narrowed what it *means*: no longer "the whole map's climate" but "which band home
 * sits in." Both bands now exist in every world -- see `climateAt()` -- so the picker chooses a
 * starting homeland rather than locking half the game's content out of the save. */
export function createInitialWorldState(
  ...
  // Issue #107: `climate` now means "which band home sits in" rather than "the whole world's
  // climate," so the picker keeps a real meaning while the map can hold both bands.
  revealNeighborsInPlace(tiles, home, (coord) => climateAt(coord, home, climate), rng);
