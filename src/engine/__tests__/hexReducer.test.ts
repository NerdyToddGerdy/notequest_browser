...
const HOME_REVEAL_ROLLS = [
  2,
  3, // {1,0}: mountain, no location
  6,
  6,
  4, // {1,-1}: plain, humanCity
  1,
  1, // {1,-1}'s name: "Iron" + "hold" -> "Ironhold"
  1,
  2, // {0,-1}: water, no location
  4,
  3, // {-1,0}: plain, no location
  4,
  3, // {-1,1}: plain, no location
  4,
  3, // {0,1}: plain, no location
];
...
describe("hexReducer MOVE", () => {
  it("moves the player onto a revealed, passable neighbor and reveals its own neighbors", () => {
    ...
    const next = hexReducer(
      world,
      { type: "MOVE", to: { q: 1, r: 0 }, raceName: "Human" },
      sequenceDie([3, 4]),
    );
  ...
  it("lets a water-walking race (Patovsky/Sharkin) cross water with no boat at all", () => {
    ...
    const next = hexReducer(
      world,
      { type: "MOVE", to: { q: 0, r: -1 }, raceName: "Sharkin" },
      sequenceDie([3, 4]),
    );
...
describe("hexReducer STORM_RELOCATE (Events on Travel, issue #91)", () => {
  it("moves the player to some other known hex, not a neighbor-only one", () => {
    ...
    const next = hexReducer(
      world,
      { type: "STORM_RELOCATE", raceName: "Human" },
      sequenceDie([3, 4]),
    );
